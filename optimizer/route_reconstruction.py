...
def _same_parking_cluster(
    ...
    if cluster_plan is None:
        return False
    ca = cluster_plan.node_to_cluster.get(a)
    cb = cluster_plan.node_to_cluster.get(b)
    if ca is None or ca != cb:
        return False
    if len(cluster_plan.clusters[ca]) < 2:
        return False
    return cluster_plan.cluster_use_parking.get(ca, False)


def _split_order_into_legs(
    order: list[int],
    ...
    *,
    ...
    for i in range(1, len(order)):
        ...
        if _same_parking_cluster(prev, curr, cluster_plan):
            ...
            not parking_mode
            and nodes
            and segment_mode_for_leg(
                travel_matrix[prev][curr],
                parking_mode=parking_mode,
                from_lat=nodes[prev]["lat"],
                from_lng=nodes[prev]["lng"],
                to_lat=nodes[curr]["lat"],
                to_lng=nodes[curr]["lng"],
            )
            == "walk"
        ):
            legs[-1].append(curr)
...
def build_parking_aware_route(
    ...
    legs = _split_order_into_legs(
        order, travel_matrix, cluster_plan, parking_mode=parking_mode, nodes=nodes
    )
    ...
    for leg in legs:
        ...
        if parking and use_hub:
            ...
            if prev_stop and prev_stop.get("kind") != "parking":
                add_travel(prev_stop, p_stop, force_mode="car")
            ...
            if prev_stop and prev_stop.get("id") != p_stop["id"]:
                ...
                ret_mode = segment_mode_for_leg(
                    ...
                    parking_mode=parking_mode,
                    prefer_walk=True,
                    from_lat=prev_stop["lat"],
                    from_lng=prev_stop["lng"],
                    to_lat=p_stop["lat"],
                    to_lng=p_stop["lng"],
                )
