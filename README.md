...
네이버지도 등에서 모은 **도로명/지번 주소**를 입력하면, OR-Tools가 방문 순서를 재구성하고 TMAP으로 이동시간을 계산하며, 카카오 Local API(PK6 주차장)와 GPT 설명을 제공합니다.

## 기능

- Place Card 입력 + TMAP Geocoding (POI fallback, 수동 좌표)
- 방문 규칙 (바로 다음 / 다음), 예약 시간, 출발 시각
- OR-Tools 최적화 + 카카오 주차장 추천 (공영·민영 혼합 후보)
...
| `KAKAO_REST_API_KEY` | 카카오 Local API (PK6 주차장) |
